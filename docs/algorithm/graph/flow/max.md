...
### 多源汇最大流
既然给了若干个源点 $s_i$ 和汇点 $t_i$，不妨让新建的 $S$ 向 $s_i$ 连容量为 $+\infty$ 的边，$t_i$ 向 $T$ 连容量为 $+\infty$ 的边。  
考虑证明新图和原图一一对应：  
1.  首先，对于原图的一个可行流，显然所有 $S$ 到 $s_i$ 的边流量为 $s_i$ 流出减去原图 $s_i$ 流入，满足流量限制，容量为 $+\infty$，所以满足容量限制。  
2.  再看新图的一个可行流，将新加的边删去以后，由于原图中 $s_i$ 和 $t_i$ 不需要满足流量守恒，所以这仍是原图的一个可行流。  
