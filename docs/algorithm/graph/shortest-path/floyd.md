...
## 固定经过边数
### Acwing 345. 牛站 {#acwing-345}
[题目传送门](https://www.acwing.com/problem/content/347/)  
由于最多只有 $100$ 条边，所以最多有 $200$ 个节点，考虑 Floyd。  
对于 Floyd 而言，它的 $f_{k,i,j}$ 表示从 $i$ 到 $j$，仅经过编号为 $1\sim k$ 的节点的最短路径长度。  
很明显，这不能满足边数的限制，所以我们将状态更改为 $f_{a+b,i,j}$ 表示从 $i$ 到 $j$，经过 $a+b$ 条边的最短路径长度，转移很明显是 $
f_{a+b,i,j}=f_{a,i,k}+f_{k,j}\ (1\leq k\leq n)$。  
但是，如果直接做复杂度是 $O(MN^3)$（这里的 $N$ 是点数），所以考虑优化。  
目前的复杂度瓶颈是每增加一条边都要重新枚举，所以考虑倍增，可以使用快速幂的思想，将 $k$ 进行二进制分解然后跑魔改后的 Floyd。  
时间复杂度 $O(N^3\log M)$。  